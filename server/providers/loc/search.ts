...
export class LocProvider implements ProviderAdapter {
  readonly id = 'loc';
  readonly name = 'Library of Congress';
  readonly description = 'National Audiovisual Conservation Center: earliest motion pictures, historic newsreels, presidential addresses, San Francisco 1906, and American memory reels';
  ...
  isConfigured(): boolean {
    return true; // 100% open national archive
  }
  ...
  async search(options: ProviderSearchOptions): Promise<ProviderSearchResult> {
    ...
    try {
      ...
      const solrQuery = `collection:(library_of_congress) AND mediatype:(movies) AND ${queryFilter}`;
      ...
      if (!res.ok) {
        return {
          items: [],
          status: 'error',
          error: `Library of Congress archive returned HTTP ${res.status}`,
        };
      }
      ...
      const items: MediaItem[] = docs.map((doc: any) => {
        ...
        const creator = doc.creator || 'Library of Congress National Archive';
        ...
        const description = descClean
          ...
          : `Library of Congress historical motion picture film recording${year}.`;
        ...
        return {
          ...
          tags: ['History', 'Library of Congress', 'American Memory', 'Documentary', 'Vintage'],
        };
      });
      ...
      return {
        items: [],
        status: 'error',
        error: err.message || 'Library of Congress query failed',
      };
    } finally {
      cleanup();
    }
