...
export class AnimationProvider implements ProviderAdapter {
  readonly id = 'animation';
  readonly name = 'Golden Age Animation Vault';
  readonly description = 'Historic vintage cartoons and hand-drawn animation milestones (Fleischer Studios, Popeye, Betty Boop, Superman 1941, Felix the Cat, Ub Iwerks)';
  ...
  isConfigured(): boolean {
    return true; // 100% open public domain animation
  }
  ...
  async search(options: ProviderSearchOptions): Promise<ProviderSearchResult> {
    ...
    try {
      ...
      const solrQuery = `collection:(animationandcartoons) AND mediatype:(movies) AND ${queryFilter}`;
      ...
      if (!res.ok) {
        return {
          items: [],
          status: 'error',
          error: `Animation archive returned HTTP ${res.status}`,
        };
      }
      ...
      const items: MediaItem[] = docs.map((doc: any) => {
        ...
        const studio = doc.creator || 'Animation Pioneers';
        ...
        const description = descClean
          ...
          : `Restored Golden Age hand-drawn animation short by ${studio}${year}.`;
        ...
        return {
          ...
          tags: ['Animation', 'Cartoons', 'Golden Age', 'Fleischer', 'Popeye', 'Vintage'],
        };
      });
      ...
      return {
        items: [],
        status: 'error',
        error: err.message || 'Animation query failed',
      };
    } finally {
      cleanup();
    }
  ...
  async resolvePlayback(id: string): Promise<PlaybackResolution | null> {
    const rawId = id.replace(/^animation:/, '');
