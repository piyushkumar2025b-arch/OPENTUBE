...
export class SportsArchiveProvider implements ProviderAdapter {
  readonly id = 'sportsarchive';
  readonly name = 'Classic Sports & Olympics Vault';
  readonly description = 'Legendary sporting milestones: Golden-age championship boxing (Muhammad Ali, Joe Louis, Frazier), historic Olympics, baseball legends & classic football';
  ...
  isConfigured(): boolean {
    return true; // 100% open sports heritage archive
  }
  ...
  async search(options: ProviderSearchOptions): Promise<ProviderSearchResult> {
    ...
    try {
      ...
      const solrQuery = `collection:(classic_sports OR sports) AND mediatype:(movies) AND ${queryFilter}`;
      ...
      if (!res.ok) {
        return {
          items: [],
          status: 'error',
          error: `Sports archive returned HTTP ${res.status}`,
        };
      }
      ...
      const items: MediaItem[] = docs.map((doc: any) => {
        ...
        const creator = doc.creator || 'Sports Heritage';
        ...
        const description = descClean
          ...
          : `Historic sports championship footage and highlights${year}.`;

        return {
          id: `sportsarchive:${id}`,
          provider: 'sportsarchive',
          title: `${title}`,
          description,
          ...
          likes: Math.round((Number(doc.downloads) || 100) * 0.15),
          playerType: 'html5',
          license: {
          ...
          tags: ['Sports', 'Olympics', 'Boxing', 'Championship', 'Athletics', 'Vintage'],
        };
      });
      ...
      return {
        items: [],
        status: 'error',
        error: err.message || 'Sports archive query failed',
      };
    } finally {
      cleanup();
    }
